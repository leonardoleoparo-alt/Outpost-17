...
export class PathRoute {
  ...
  draw(ctx) {
    ...
    this.stroke(ctx, this.width + 26, '#56794e');
    this.stroke(ctx, this.width + 15, '#9c7a4c');
    this.stroke(ctx, this.width, '#d5ae72');
    this.stroke(ctx, this.width - 16, '#dcb980');

    ctx.globalAlpha = .28;
    ctx.setLineDash([7, 17]);
    this.stroke(ctx, 3, '#916e42');
    ctx.setLineDash([]);
    ctx.restore();
  }
